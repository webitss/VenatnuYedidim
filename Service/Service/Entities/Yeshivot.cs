...
namespace Service.Entities
{
	...
	public class Yeshivot
	{
		...
		public static bool DeleteYeshivaOfStudent(int iPersonId, int iYeshivaId, int iUserId)
		{
			try
			{
				List<SqlParameter> parameters = new List<SqlParameter>();
				...
				parameters.Add(new SqlParameter("iYeshivaId", iYeshivaId));
				SqlDataAccess.ExecuteDatasetSP("TStudentYeshiva_DEL", parameters);

				return true;
			}
			catch (Exception ex)
			{
				Log.LogError("DeleteYeshivaOfStudent/TStudentYeshiva_DEL", "ex" + ex);
				return false;
			}
		}
